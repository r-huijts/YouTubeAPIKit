...
public struct VideoInfosWithDownloadFormatsResponse: YouTubeResponse {
    ...
    public typealias VideoDownloadFormat = YouTubeAPIKit.VideoDownloadFormat
    ...
    public typealias AudioOnlyFormat = YouTubeAPIKit.AudioOnlyFormat
    ...
    private static func processPlayerScrapping(playerPath: String) throws -> (instructions: [PlayerCipherDecodeInstruction], nParameterCode: String) {
        ...
        if
            let savedPlayerInstructionsData = FileManager.default.contents(atPath: documentDirectoryPath + "YouTubeAPIKitPlayers-\(playerName).ab"),
            let savedPlayerIntructions = try? JSONDecoder().decode([PlayerCipherDecodeInstruction].self, from: savedPlayerInstructionsData),
            let savedPlayerCodeData = FileManager.default.contents(atPath: documentDirectoryPath + "YouTubeAPIKitPlayers-\(playerName).abn")
        {
        ...
        for line in separatedByLinePlayer {
            ...
            do {
                FileManager.default.createFile(
                    atPath: documentDirectoryPath + "YouTubeAPIKitPlayers-\(playerName).ab",
                    contents: try JSONEncoder().encode(instructionsArray)
                )
                
                FileManager.default.createFile(
                    atPath: documentDirectoryPath + "YouTubeAPIKitPlayers-\(playerName).abn",
    ...
    static func decodeFormatFromJSON(json: JSON) -> DownloadFormat {
        if json["fps"].int != nil {
            ...
            return YouTubeAPIKit.VideoDownloadFormat(
            ...
            return YouTubeAPIKit.AudioOnlyFormat(
                averageBitrate: json["averageBitrate"].int,
                contentLength: {
                    if let contentLength = json["contentLength"].string {
                        return Int(contentLength)
    ...
    /// Get the document directory from the project that is using ``YouTubeAPIKit``
